...
class Links
{
    private:
        ...
        T last_free_link;
    public:
        Links(const char* data_file, const char* index_file);
        ~Links();
...
template <typename T>
Links<T>::Links(const char* data_file, const char* index_file) : data_array(data_file), index_array(index_file) {
